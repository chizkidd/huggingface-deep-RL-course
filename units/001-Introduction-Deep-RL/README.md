...
2. **Value-Based Methods:** The agent learns a **Value Function** ($Q(s,a)$ or $V(s)$) that maps a state to the expected return. The policy then becomes: "Take the action that leads to the state with the highest value."
