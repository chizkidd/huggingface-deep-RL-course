...
2. **Value-Based Methods:** The agent learns a **Value Function** ($V(s)$ or $Q(s,a)$, $\mathbf{X}_{0}, \dots, \mathbf{X}_{\ell}-1$) that maps a state to the expected return. The policy then becomes: "Take the action that leads to the state with the highest value."
